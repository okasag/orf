...
as in @Okasa2019. The Ordered Forest flexibly
...
In order to install `orf` type

```{r install, eval = FALSE}
#install.packages("orf", dependencies = c("Depends", "Suggests"))
```

to make sure that all packages needed are installed.
...
Additionally, `orf` supports standard `R` functions such as `summary`, `predict`,
...
First, load an example data included in the `orf` package. This data includes a 
ordered categorical outcome variable with 3 distinct ordered classes ($Y\in\{1,2,3\}$)
with a set of four covariates ($X1, X2, X3, X4$) of different types. The first
...
within the data generating process, covariates $X1$, $X2$ and $X3$ are all linear 
and all feature a positive effect on the outcome, while $X4$ is without any effect
and thus a noise variable in the dataset. For the exact DGP, see `orf::?odata`.
...
accuracy measured as in terms of the classical mean squared error (MSE) and the 
...
contrastto the actually observed outcome class and as such provide a visual 
inspection of theunderlying probability predictions for the outcome classes.
...
orf_fitted <- predict(orf)

# print orf fitted values
print(orf_fitted)
```

Now, divide the data into train and test set for a prediction exercise and 
summarize the prediction results. Similarly to the above, also for the prediction
...
orf_pred <- predict(orf, newdata = X_test, type = "probs", inference = FALSE)
...
the share of standard deviation of the particular covariate $X$ with default equal
to `window = 0.1`. Furthermore, new data for which marginal effects should be
...
down, respectively. For a detailed discussion see @Okasa2019.
...
orf_margins <- margins(orf, eval = "mean", window = 0.1,
